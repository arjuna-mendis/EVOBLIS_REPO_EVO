import numpy as np
import DistortionTools      
import cv2
from DistortionTools import Camera
def calibrationExample():
    camNum =1           # The number of the camera to calibrate
    nPoints = 5         # number of images used for the calibration (space presses)
    patternSize=(9,6)   #size of the calibration pattern
    saveImage = False

    calibrated, camera_matrix,dist_coefs,rms = DistortionTools.calibrateCamera(camNum,nPoints,patternSize,saveImage)
    K = camera_matrix
