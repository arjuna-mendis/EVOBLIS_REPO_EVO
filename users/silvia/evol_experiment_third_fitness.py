...
class EvolutionaryExperimentThirdFitness:
    ...
    def perform_experiment(self, petri, ph, molarity, gen, individual_num, replica_num):
        ...
        while True:
            ...
            # Get path
            is_droplet, path = track2.get_path_biggest_init()
            ...
            if distance_from_start_point < radius_start and not left_safe_area:
                ...
                if counter % analysis_frame == 0.0:
                    ...
                    if len(path) - 1 - analysis_frame - frame_droplet_left_safe > -1:
                        ...
                        salt_dir = math.atan2(salt_destination_pixel[1] - last_pos[1], salt_destination_pixel[0] - last_pos[0])
            ...
            print 'this is the current position:' + str(current_pos)
            print 'this is the petri center:' + str(petri.center)
            distance_droplet_from_center = _calculate_pitagora((self.x_in_pixels, self.y_in_pixels), current_pos)  # pix
            print 'this is the distance from the center of the petri: %f' % distance_droplet_from_center

            _, _, droplet_area = track2.show_output_biggest(result)  # in pixel
            print 'This is the new droplet area: %f' % droplet_area

            droplet_diameter = math.sqrt(droplet_area / 3.14) * 2.0
            print 'this is the diameter of the droplet: %f' % droplet_diameter
            # radius_mm = 45.0

            limit_edge = radius_pix - droplet_diameter
            print 'this is the limit edge: %f' % limit_edge

            if distance_droplet_from_center > limit_edge:
                print 'at border'
                end_tracking_time = datetime.datetime.now()
                break
            ...
            if modality == "tracking":
                ...
                cv2.putText(result,
                            "ph: %f, mol: %f mM, generation: %d, ind_num: %d, replica: %d, current_fitness: %f" %
                            (round(ph, 2), round(molarity, 2), gen, individual_num, replica_num, fitness_value),
        ...
        if frame_counter == 0.0:
            print 'the frame counter is equal to zero'
            fitness_value = -0.05
        else:
            fitness_value = fitness_value / (frame_counter + 100)

        print 'This is the final fitness value registered: %f' % fitness_value
        ...
        if recording:
            import os
            date = str(datetime.datetime.now())
            file_name = os.path.join(FILE_PATH, "%s_fitness%f.jpg" % (vT.removeColon(date + file_name_addition),
                                                                      fitness_value))
        ...
        import csv
        with open('/Users/capo/Documents/data_from_exp/26July' + file_name_addition + "-{}.csv".format(
