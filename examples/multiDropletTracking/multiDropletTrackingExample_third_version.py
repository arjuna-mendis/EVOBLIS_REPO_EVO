...
__author__ = "capo"
__date__ = "$24-Jul-2017 15:03:00$"

number = 15

if __name__ == "__main__":
    print "Multi Droplet Tracking Example with Fitness computed every step, plus other contraints + " \
          "number individual %d" % number

path_file = '/Users/capo/Desktop/my_folder/peg_third_fitness/'
...
if number == 0:
    ...
    petridishCenter = (519, 450)  # 1 #triton
    x_center, y_center = (193, 252)  # 1
elif number == 9:
    petridishCenter = (519, 450)  # 1
    x_center, y_center = (193, 252)  # 1
elif number == 2:
    ...
    petridishCenter = (518, 170)  # 13 tritonx
    ...
    petridishCenter = (799, 170)  # 14
    x_center, y_center = (97, 350)  # 14
else:
    petridishCenter = (1080, 170)  # 7
...
while True:
    ...
    if distance_from_start_point < radius_start and not left_safe_area:
        ...
        if counter % analysis_frame == 0.0:
            ...
            if len(path) - 1 - analysis_frame - frame_droplet_left_safe > -1:
                ...
                print 'this is the angle difference: %f ' % abs(angle_dif)
                print 'this is the angle threshold: %f' % angle_threshold
            ...
            # check is the droplet is close to the salt and stop tracking if it is
            # if salt_distance < distance_threshold:
            #     print "Tracking stopped, droplet has gone in the salt area"
            #     end_tracking_time = datetime.datetime.now()
            #     break

        # initPos, endPos, droplet_area = trackObject.show_output_biggest(result)
    distance_from_center = _compute_distance(petridishCenter, current_pos)
    print 'this is the distance from the center of the petri: %f' % distance_from_center

    _, _, droplet_area = trackObject.show_output_biggest(result)  # in pixel
    ...
    limit_edge = petridishDiameter - droplet_diameter
    print 'this is the limit edge: %f' % limit_edge

    if distance_from_center > limit_edge:
        print 'at border'
        end_tracking_time = datetime.datetime.now()
        break
...
if frame_counter == 0.0:
    ...
    fitness_value = fitness_value / (frame_counter + 10)
